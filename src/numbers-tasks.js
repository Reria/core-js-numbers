...
function getAverage(value1, value2) {
  ...
  // Если оба значения конечны, возвращаем обычное среднее значение
...
function toPrecision(number, precision) {
  return Number.parseFloat(number.toPrecision(precision)).toString();
}
...
function isNumber(number) {
  return (
    (number instanceof Number || typeof number === 'number') &&
...
function getCountOfOddNumbers(number) {
  const positiveNumber = Math.floor(Math.abs(number));
  // Calculate the count of odd numbers using integer division
  const count = Math.floor((positiveNumber + 1) / 2);
  return count;
}
